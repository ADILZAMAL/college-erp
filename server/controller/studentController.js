...
module.exports = {
  ...
  getAllStudents: async (req, res, next) => {
    ...
    try {
      ...
      const students = await Student.find({ std: department, section });
  ...
  getMarks: async (req, res, next) => {
    try {
      ...
      const getMarks = await Mark.find({ student: id }).populate("subject");
      console.log("getMarks", getMarks);

      const formativeAssessment1 = getMarks.filter((obj) => {
        return obj.exam === "Formative Assessment - 1";
      });
      const formativeAssessment2 = getMarks.filter((obj) => {
        return obj.exam === "Formative Assessment - 2";
      });
      const submitiveAssessment1 = getMarks.filter((obj) => {
        return obj.exam === "Submitive Assessment - 1";
      });
      const submitiveAssessment2 = getMarks.filter((obj) => {
        return obj.exam === "Submitive Assessment - 2";
      });
      res.status(200).json({
        result: {
          formativeAssessment1,
          formativeAssessment2,
          submitiveAssessment1,
          submitiveAssessment2,
        },
      });
    } catch (err) {
